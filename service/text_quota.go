...
// call), or settle a pre-consumed amount when upstream usage is missing.
func (s *textQuotaSummary) hasBillableUsage() bool {
	return s.TotalTokens > 0 || s.MissingUsagePreConsumed || !s.ToolCallSurchargeQuota.IsZero()
}
...
func calculateTextQuotaSummary(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage) textQuotaSummary {
	...
	if summary.TotalTokens == 0 && missingUsage {
