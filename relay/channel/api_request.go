...
func applyRSGatewayIdentityHeaders(header http.Header, c *gin.Context, info *common.RelayInfo) {
	...
	header.Del("X-Gateway-NewAPI-Token-Name")
	header.Del("X-RS-NewAPI-Using-Group")
	header.Del("X-RS-NewAPI-Group-Ratio")
	header.Del("X-RS-NewAPI-Request-ID")
	if c == nil || c.Request == nil {
		return
	}
	...
	if info.RequestId != "" {
		header.Set("X-RS-NewAPI-Request-ID", info.RequestId)
	}
	...
	if usingGroup := strings.TrimSpace(info.UsingGroup); usingGroup != "" {
		groupValue := url.PathEscape(usingGroup)
		if tokenName != "" {
			groupValue += ";x-token-name=" + url.PathEscape(tokenName)
		}
		header.Set("X-RS-NewAPI-Using-Group", groupValue)
