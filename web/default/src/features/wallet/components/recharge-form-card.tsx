...
const PAYMENT_SKELETON_KEYS = ['payment-1', 'payment-2', 'payment-3']
...
export function RechargeFormCard({
  ...
  const hasMultipleRedemptionLines =
    redemptionCode.includes('\n') || redemptionCode.includes('\r')
        ...
        <div className='space-y-2.5 border-t pt-4 sm:space-y-3 sm:pt-6'>
          ...
          <div className='grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center'>
            <Textarea
              ...
              placeholder={
                hasMultipleRedemptionLines
                  ? t('Enter redemption codes, one per line')
                  : t('Enter your redemption code')
              }
              rows={hasMultipleRedemptionLines ? 3 : 1}
              spellCheck={false}
              disabled={redeeming}
              className={cn(
                'min-w-0',
                hasMultipleRedemptionLines
                  ? 'min-h-20 resize-y'
                  : 'h-9 min-h-9 max-h-9 resize-none overflow-hidden py-1'
              )}
            />
            <Button
              onClick={onRedeem}
